...
<body>
    ...
    <div id="boutonAcceuil">
        <nav class="nav bg-light justify-content-center">
            ...
            <a href="<?php if(!isset($_SESSION['User'])) echo Router::url("user/connexion"); else echo Router::url("user/logout");?>" class="nav-link text-dark">
                <?php 
                    if(!isset($_SESSION['User'])) echo 'Se connecter';
                    else echo 'Se déconnecter'
                ?>
            </a> &nbsp;
            <a href="<?php if(isset($_SESSION['User'])) echo Router::url("panier/index/id:{$_SESSION['User']->id}"); else echo "#";?>" class="nav-link text-dark">
